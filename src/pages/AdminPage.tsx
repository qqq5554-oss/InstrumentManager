...
type Tab = 'instruments' | 'employees' | 'notifications'
  ...
  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
        <h1 className="text-2xl font-bold text-gray-900 shrink-0">管理後台</h1>
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={runScan}
            disabled={scanning}
            className="flex items-center gap-1.5 whitespace-nowrap px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium border border-indigo-300 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 disabled:opacity-50 transition-colors"
          >
            🔍 {scanning ? '掃描中…' : '立即掃描'}
          </button>
          <button
            ...
            className="flex items-center gap-1.5 whitespace-nowrap px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium border border-blue-300 bg-blue-50 text-blue-700 hover:bg-blue-100 disabled:opacity-50 transition-colors"
          >
            🔔 {pushTesting ? '發送中…' : '測試通知'}
          </button>
          <button
            onClick={toggleTestMode}
            className={`flex items-center gap-1.5 whitespace-nowrap px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium border transition-colors ${
              testMode
                ? 'bg-amber-50 border-amber-300 text-amber-700 hover:bg-amber-100'
                : 'bg-green-50 border-green-300 text-green-700 hover:bg-green-100'
            }`}
          >
            <span className="text-gray-500 font-normal">LINE：</span>
            {testMode ? '🧪 測試' : '✅ 正式'}
          </button>
        </div>
      </div>
      <div className="flex gap-1 mb-5 border-b border-gray-200">
        {([['instruments', '儀器管理'], ['employees', '人員管理'], ['notifications', '通知管理']] as [Tab, string][]).map(([key, label]) => (
      ...
      {tab === 'instruments' ? <InstrumentsTab /> : tab === 'employees' ? <EmployeesTab /> : <NotificationsTab />}
    </div>
  )
}
